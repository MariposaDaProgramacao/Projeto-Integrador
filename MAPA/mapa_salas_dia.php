<?php
// ==========================================================
// mapa_salas_dia.php - Mapa de Salas por Data (MODIFICADO PARA MULTI-TENANT)
// ==========================================================
...
try {
    ...
    // ============================================================
    // 6. CONTAGEM DE SALAS POR STATUS
    // ============================================================
    $contagemStatus = [
        'disponivel' => 0,
        'ocupada' => 0,
    ...
    foreach ($salas as $sala) {
        ...
        if (isset($manutencoesPorSala[$sala['id_sala']])) {
            $contagemStatus['manutencao']++;
        } elseif ($status === 'inativa') {
            $contagemStatus['inativa']++;
        } else {
            $contagemStatus['disponivel']++;
        }
    }

    $totalOcupadas = 0;
    foreach ($agenda as $item) {
        foreach ($item['aulas'] as $aula) {
            if ($aula !== null) {
                $totalOcupadas++;
                break;
            }
    ...
    $totalManutencao = $contagemStatus['manutencao'];
    $totalInativas = $contagemStatus['inativa'];
    $contagemStatus['disponivel'] = $totalSalas - $totalOcupadas - $totalManutencao - $totalInativas;
    if ($contagemStatus['disponivel'] < 0) $contagemStatus['disponivel'] = 0;

    // ============================================================
    // 7. FILTROS DE BUSCA, STATUS E TIPO
    // ============================================================
    if (!empty($busca) || !empty($statusSalaFiltro) || !empty($tipoSalaFiltro)) {
        ...
        foreach ($agenda as $idSala => $item) {
            ...
            $statusExibicao = $emManutencaoNaData ? 'manutencao' : $statusSala;

            if (!empty($statusSalaFiltro)) {
                if ($statusExibicao !== $statusSalaFiltro) {
                    continue;
                }
            }
            ...
            if (!empty($busca)) {
                ...
                if (!$encontrou) {
                    foreach ($item['aulas'] as $turno => $aula) {
                        if ($aula !== null) {
                            $nomeCurso = $aula['nome_curso'] ?? '';
                            $numeroCurso = $aula['numero_curso'] ?? '';
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .btn-mapa {
            ...
            text-decoration: none;
            box-shadow: 0 8px 24px -6px rgba(26, 115, 232, 0.45);
            margin-bottom: 20px;
            align-self: flex-start;
        }
        .btn-mapa:hover {
            transform: translateY(-3px);
            box-shadow: 0 12px 32px -8px rgba(26, 115, 232, 0.55);
            background: linear-gradient(145deg, #1557b0, #0a3a7a);
        }
        .btn-mapa i {
            font-size: 22px;
        }
...
<body>
    ...
    <main class="main">
        ...
        <!-- BOTÃO: VER TODO O MAPA DE SALAS -->
        <a href="mapa_sala.php" class="btn-mapa">
            <i class="fas fa-map"></i>
            Ver todo o mapa de salas
            <span class="badge-mapa">📍 Todas as salas</span>
        </a>
            ...
            <section class="turno-section <?php echo $classeTurno; ?>">
                ...
                <div class="turno-cards">
                    ...
                    foreach ($agenda as $item):
                        ...
                        $emManutencaoNaData = isset($manutencoesPorSala[$sala['id_sala']]);
                        $motivoManut = '';
                        if ($emManutencaoNaData) {
                            $motivoManut = $manutencoesPorSala[$sala['id_sala']]['motivo'] ?? '';
                        }
